...
def download_data_upbit(symbol, start_date, end_date):
    ...
    while current_date > start_date:
        ...
        if response.status_code == 200:
            ...
            current_date = datetime.strptime(new_data[-1]['candle_date_time_kst'], '%Y-%m-%dT%H:%M:%S')
            print(f"데이터 수집 중: {current_date}")
        else:
            break
...
# Now, modify the MRHATradingSystem to accept and preprocess the Upbit data
class MRHATradingSystem:
    def __init__(self, symbol, start_date, end_date, source='yfinance'):
        self.symbol = symbol
    ...
    def download_data(self):
        ...
        elif self.source == 'upbit':
            self.stock_data = download_data_upbit(self.symbol, self.start_date, self.end_date)
...
def preprocess_codes(file_path, market_type):
    ...
    elif market_type == "UPBIT":
        code_column = df.columns[0]  # UPBIT 코드가 첫 번째 열에 있다고 가정
    else:
        raise ValueError("Unsupported market type. Please use 'ETF', 'KOSPI', or 'UPBIT'.")
...
def check_buy_signal(etf, end_date, source='yfinance'):
    start_date = end_date - timedelta(days=60)  # 60일 데이터로 충분한 이력 확보
    
    try:
        trading_system = MRHATradingSystem(etf, start_date, end_date, source=source)
